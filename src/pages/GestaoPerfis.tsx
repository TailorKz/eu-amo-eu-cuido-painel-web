...
export default function GestaoPerfis() {
  ...
  const carregarUsuarios = async () => {
    try {
      const response = await axios.get(`https://tailorkz-production-eu-amo.up.railway.app/api/cidadaos/cidade/${cidadeAdmin}`);
  ...
  const carregarSetores = async () => {
    try {
      const response = await axios.get(`https://tailorkz-production-eu-amo.up.railway.app/api/setores?cidade=${cidadeAdmin}`);
  ...
  const abrirDetalhes = (usuario: Usuario) => {
    if (!isSuperAdmin && usuario.perfil === "SUPER_ADMIN") return alert("Acesso negado. Utilizador de nível superior.");
    if (!isSuperAdmin && usuario.perfil === "GESTOR_SETOR" && usuario.setorAtuacao !== meuSetor) return alert("Não pode editar um gestor de outro setor.");
  ...
  const handleSalvarPerfil = async () => {
    ...
    try {
      ...
      const setorFinal = (novoPerfil === "CIDADÃO" || novoPerfil === "SUPER_ADMIN" || novoPerfil === "VEREADOR") ? null : novoSetor;

      await axios.put(url, { perfil: novoPerfil === "CIDADÃO" ? null : novoPerfil, setorAtuacao: setorFinal });
  ...
  const handleAlternarBloqueio = async (bloquear: boolean) => {
    ...
    setIsSaving(true);
    try {
      await axios.put(`https://tailorkz-production-eu-amo.up.railway.app/api/cidadaos/${usuarioSelecionado.id}/bloquear?bloquear=${bloquear}`);
  ...
  const handleExcluirUsuario = async () => {
    ...
    setIsSaving(true);
    try {
      await axios.delete(`https://tailorkz-production-eu-amo.up.railway.app/api/cidadaos/admin-excluir/${usuarioSelecionado.id}`);
  ...
  const handleAdicionarMembro = async () => {
    if (!addTelefone || addTelefone.length < 11) return alert("Preencha corretamente o número de celular (11 dígitos)!");
  ...
  const usuariosFiltrados = usuarios.filter((u) => {
    const matchBusca = u.nome.toLowerCase().includes(termoBusca.toLowerCase()) || u.telefone.includes(termoBusca);
    ...
    <div className="flex h-screen overflow-hidden bg-gray-100">
      <aside className="w-64 bg-white shadow-md flex flex-col z-10">
        ...
        <nav className="flex-1 px-4 space-y-2 mt-4">
          ...
          {isSuperAdmin && (
            <a href="#" onClick={(e) => { e.preventDefault(); navigate("/definicoes"); }} className="flex items-center gap-3 p-3 text-gray-600 hover:bg-gray-50 rounded-lg transition-colors">
              <Settings size={20} /> Definições
            </a>
          )}
        </nav>
        <div className="p-4 border-t border-gray-100">
          <button onClick={() => { localStorage.removeItem("user_ipora"); navigate("/"); }} className="flex items-center gap-3 p-3 text-red-500 hover:bg-red-50 rounded-lg transition-colors w-full">
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        <header className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="flex gap-3">
            ...
            <button onClick={() => setIsAddModalOpen(true)} className="flex items-center gap-2 bg-primary text-white px-4 py-2 rounded-lg font-bold hover:bg-primaryDark transition-colors shadow-sm">
            ...
            <tbody className="divide-y divide-gray-100">
              {isLoading ? (
                ...
                usuariosFiltrados.map((user) => (
                  <tr key={user.id} onClick={() => abrirDetalhes(user)} className={`cursor-pointer transition-colors ${user.bloqueado ? "bg-red-50/40 hover:bg-red-50" : "hover:bg-blue-50"}`}>
                    <td className="p-4 font-medium text-gray-800 flex items-center gap-3">
                      ...
                      <span className={user.bloqueado ? "line-through text-gray-400" : ""}>{user.nome}</span>
                      {user.bloqueado && (
                        <span title="Usuário Banido" className="flex items-center">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <select value={addPerfil} onChange={(e) => setAddPerfil(e.target.value)} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary outline-none bg-white">
              ...
              <div>
                ...
                <select value={addSetor} onChange={(e) => setAddSetor(e.target.value)} disabled={!isSuperAdmin} className={`w-full p-3 border rounded-lg outline-none ${!isSuperAdmin ? "bg-gray-100 border-gray-200 text-gray-500" : "bg-white border-gray-300 focus:ring-2 focus:ring-primary"}`}>
                  {!isSuperAdmin ? (
              ...
              <button onClick={handleAdicionarMembro} disabled={isCreatingUser} className="w-full mt-4 py-3 bg-primary text-white font-bold rounded-xl hover:bg-primaryDark transition-colors disabled:opacity-50">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="flex justify-between items-center p-6 border-b border-gray-100 bg-gray-50">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xl font-bold text-gray-800">Gerir Utilizador</h2>
              </div>
            ...
            <div className="p-6 space-y-6">
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {usuarioSelecionado.bloqueado && <span className="bg-red-100 text-red-700 text-xs px-2 py-0.5 rounded-full font-bold">Banido</span>}
                </div>
              ...
              <div className={`p-4 rounded-xl border ${usuarioSelecionado.bloqueado ? "bg-gray-50 border-gray-200 opacity-60 pointer-events-none" : "border-blue-100 bg-blue-50/30"}`}>
                ...
                <select value={novoPerfil} onChange={(e) => setNovoPerfil(e.target.value)} className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary outline-none bg-white">
                ...
                {(novoPerfil === "FUNCIONARIO" || novoPerfil === "GESTOR_SETOR") && (
                  <div className="mt-4 animate-in fade-in slide-in-from-top-2">
                    ...
                    <select value={novoSetor} onChange={(e) => setNovoSetor(e.target.value)} disabled={!isSuperAdmin} className={`w-full p-3 border rounded-lg outline-none ${!isSuperAdmin ? "bg-gray-100 border-gray-200 text-gray-500" : "bg-white border-gray-300 focus:ring-2 focus:ring-primary"}`}>
                      {!isSuperAdmin ? (
                        <option value={meuSetor}>{meuSetor}</option>
                      ) : (
                        <>
                ...
                <div className="pt-4 border-t border-gray-100">
                  ...
                  <div className="flex flex-col gap-2">
                    {usuarioSelecionado.bloqueado ? (
                      <button onClick={() => handleAlternarBloqueio(false)} disabled={isSaving} className="w-full flex items-center justify-center gap-2 py-3 bg-emerald-50 text-emerald-600 hover:bg-emerald-100 border border-emerald-200 font-bold rounded-xl transition-colors disabled:opacity-50">
                        <CheckCircle size={18} /> Restaurar Acesso do Utilizador
                      </button>
                    ) : (
                      <button onClick={() => handleAlternarBloqueio(true)} disabled={isSaving} className="w-full flex items-center justify-center gap-2 py-3 bg-red-50 text-red-600 hover:bg-red-100 border border-red-200 font-bold rounded-xl transition-colors disabled:opacity-50">
                        <Ban size={18} /> Banir do Município
                      </button>
                    )}

                    <button onClick={handleExcluirUsuario} disabled={isSaving} className="w-full flex items-center justify-center gap-2 py-3 bg-white text-gray-700 hover:bg-red-50 hover:text-red-600 hover:border-red-200 border border-gray-200 font-bold rounded-xl transition-colors disabled:opacity-50">
